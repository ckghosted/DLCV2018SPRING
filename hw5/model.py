...
class LSTM(object):
    ...
    def build_model(self, feature_dim=40960):
        ...
        outputs = tf.stack(outputs) ### np.array with shape (self.max_seq_len, batch_size, feature_dim)
        outputs = tf.transpose(outputs, [1, 0, 2]) ### np.array with shape (batch_size, self.max_seq_len, feature_dim)
