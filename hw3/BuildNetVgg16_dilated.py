...
class NET_VGG16_DILATED:
    def __init__(self,
                 sess,
                 vgg16_npy_path,
                 model_name='VGG16_DILATED',
    ...
    def build_model(self):
        ...
        self.label_true = tf.placeholder(tf.int32, shape=[None, self.img_size//8, self.img_size//8, 1], name="label_true")
        ...
        #### Layer 5
        self.conv5_1 = self.dilated_conv_layer(self.conv4_3, rate=2, name="conv5_1")
        self.conv5_2 = self.dilated_conv_layer(self.conv5_1, rate=2, name="conv5_2")
        self.conv5_3 = self.dilated_conv_layer(self.conv5_2, rate=2, name="conv5_3")
        #### ------------------------------------------------------------
        ...
        self.label_predict = tf.argmax(self.conv8, axis=3, name="label_predict")
    ...
    def train(self,
        ...
        for epoch in range(nEpochs):
            ...
            for i in tqdm.tqdm(range(nBatches)):
                ...
                batch_label_true_small = np.zeros([batch_label_true.shape[0], self.img_size//8, self.img_size//8, 1], dtype=np.int)
                for f in range(batch_label_true.shape[0]):
                    batch_label_true_small[f, :, :, 0] = skimage.transform.resize(batch_label_true[f, :, :, 0],
